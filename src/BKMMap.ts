export class BKMMap<K, V> extends Map<K, V> {
  ...
  public computeIfAbsent(key: K, callback: (p: K) => V): V {
    if(!this.has(key)) {
  ...
  public has(key: K): boolean {
    ...
    return Array.from(this).some(([k, v]) => {
      return (k as any).equals(key)
    })
  }
  ...
  equals(compareTo: Object): boolean {
    if(compareTo === this) {
      return true;
    } else if(compareTo instanceof BKMMap) {
      return false;
    }
